...
**AI가 먼저 읽을 것은 [`docs/00-START.md`](docs/00-START.md)다.** 문 하나만 둔다. 거기에 읽는 순서·문서 지도·지금 막힌 것·하지 말 것이 있다.

코드도 설계 근거(헌장·명세·스키마·디자인·작업 순서)도 **전부 이 레포에 있다.** 2026-09-16까지는 설계 문서가 Obsidian 볼트에 따로 있었고, 옛 문장이 「볼트 `Works/비주얼파이프라인/`이 정본」이라고 말하면 그 문장이 낡은 것이다.
