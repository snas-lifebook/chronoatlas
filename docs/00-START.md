...
볼트에 일부러 남긴 것은 셋이고, 셋 다 이 레포에서 작업하는 데 필요하지 않다.

| 볼트에 남은 것 | 왜 |
|---|---|
| River가 실제로 친 프롬프트 원문 (`Context/`) | 공개 레포에 올릴 것이 아니다 |
| 시각 레퍼런스 캡처 (`assets/ref/`) | 제3자 제품 화면이라 재배포하지 않는다 |
...
라운드 A~H 중 **E·F·H는 닫혔고 G는 부분**이다. 자세한 것은 `BACKLOG.md` 「지금 순서」.

| | 무엇 | 상태 |
|---|---|---|
| A | 지도 범위 확대 | 준비 끝. **River 터미널 재베이크 대기** |
| B·C·D | 세력 실명·경계·LOD | A에 막혀 있다 |
| E | 북마크·프로젝트 | ● 2026-09-11 |
| F | 타임라인의 그 해 | ● 2026-09-11 |
| G | 말판·인물 위치·자석 | ◐ 2026-09-12. 목각 GLB 남음 |
| H | 데이터 결손·회귀 | ◐ 코드는 닫혔고 정본이 River 대기 |

**선행 없이 집을 수 있는 코드 작업은 사실상 없다.** 새로 들어왔다면 먼저 `BACKLOG.md`를 읽고, 무엇이 막혀 있는지부터 확인하라.
