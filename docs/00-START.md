...
1. **이 문서** (지금 보는 것) — 지도
2. [`CONSTITUTION.md`](CONSTITUTION.md) — **규칙. 다른 모든 것보다 위다.** 특히 0-3 「AI는 정본에 직접 쓰지 않는다」 · 0-4 **「같은 사실을 두 문서에 적지 않는다」** · 3 「관계를 지어내지 않는다」
...
| 2026-09-17 전면 개선(스킨·영토·북마크·세부 지도·모바일·장기말·입체 지형) | [`OVERHAUL.md`](OVERHAUL.md) §0. 네 슬라이스, 전부 닫힘 |
| 포인트 01~05·09~11 지도(발표 그룹 셋·세부 지도 여섯·교보재 `p12/p345/p911-*`) | [`POINTS-01-11.md`](POINTS-01-11.md). 규칙 §1·파일 규약 §4·제안 검토표 §7. 세부 지도 근거는 `ZAMA`·`AEGATES`·`ACTIUM`·`PHILIPPI`·`CAPRI.md` |
...
방향만 적는다. **2026-09-17부터 전면 개선 슬라이스 I(`OVERHAUL.md`)를 집행 중이다.** P-Z(도식 초안)·P-A(정본 마이그레이션·범위 확대)·P-B(영역 마감)·P0(번들)·P1(미시지도 레지스트리)·P2(DEM·토지피복)·P3(미시지도 셋)·P4(전투 재생)·P5(장면 둘)·P6(즉석 북마크)·P7(모바일 읽기 모드)·P-Z(도식 생성기)까지 커밋(**미푸시**). 슬라이스 I은 2026-09-17 밤에 전부 닫혔고(R46 396 kB, R56 고도색·insets.json), **슬라이스 II(시각 문법, R51)도 같은 밤에 닫혔다**(`OVERHAUL-II.md`, River 승인 넷). **슬라이스 III(`OVERHAUL-III.md`: 위성 스킨·세력 존속연도·LOD 5단)과 IV(`OVERHAUL-IV.md`: 장면 ↔ 사건 링크 층·북마크 묶음·자료실 역링크)도 같은 밤에 닫혔다.** 네 슬라이스 전부 끝, **2026-09-18 라이브 배포**. 남은 것은 BACKLOG 「지도 다음」과 River 판정 항목뿐이다. 이 환경은 egress가 열려 있어 재베이크·DEM을 에이전트가 직접 돌린다.
...
- (2026-09-17에 풀림: `migrate_v2.py --write`는 River 승인으로 집행했고, 범위 `[-25,12,75,62]` 재베이크와 DEM 굽기는 egress가 열려 에이전트가 돌린다)
- `proposals/` 검토·병합
