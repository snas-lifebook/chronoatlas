...
방향만 적는다. **2026-09-17부터 전면 개선 슬라이스 I(`OVERHAUL.md`)를 집행 중이다.** P-Z(도식 초안)·P-A(정본 마이그레이션·범위 확대)·P-B(영역 마감)·P0(번들)·P1(미시지도 레지스트리)·P2(DEM·토지피복)·P3(미시지도 셋)·P4(전투 재생)·P5(장면 둘)·P6(즉석 북마크)·P7(모바일 읽기 모드)·P-Z(도식 생성기)까지 커밋(**미푸시**). 슬라이스 I에서 남은 것은 둘: 초기 JS 410 → 400(R46)과 R56 잔여(스킨 고도색·인셋 하천·`data/insets.json`). 그다음은 슬라이스 II(`OVERHAUL.md` §2). 이 환경은 egress가 열려 있어 재베이크·DEM을 에이전트가 직접 돌린다.
