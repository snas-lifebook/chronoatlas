...
방향만 적는다. **2026-09-17부터 전면 개선 슬라이스 I(`OVERHAUL.md`)를 집행 중이다.** P-Z(도식 초안)·P-A(정본 마이그레이션·범위 확대)·P-B(영역 마감)·P0(번들)·P1(미시지도 레지스트리)·P2(DEM·토지피복)·P3(미시지도 셋)·P4(전투 재생)까지 커밋(**미푸시**). 남은 것은 P5 장면·P6 북마크·P7 모바일·P-Z 생성기. **초기 JS가 449.9/450 kB로 게이트 끝에 있으니 UI를 더 얹기 전에 지연 청크부터**(P0 라체트). 이 환경은 egress가 열려 있어 재베이크·DEM을 에이전트가 직접 돌린다.
