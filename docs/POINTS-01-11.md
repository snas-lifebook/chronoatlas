...
`proposals/20260921_p12.jsonl` 12 op · `20260921_p345.jsonl` 24 op · `20260921_p911.jsonl` 21 op. 전부 좌표는 Pleiades pid나 위키데이터 QID를 `ext`에 적었다. **병합 도구가 아직 없다**(`add_entity`·`add_link`만 있던 선례에 `set_location`·`update_entity`가 새로 들어왔다) → 승인되면 `scripts/apply-proposals.py`(결정론, 백업, 멱등)를 만들어 집행한다.
