...
「지도를 더 넓게」를 실행하는 절차. egress가 필요한 부분이 있다. **2026-09-17 실측: 이 작업 환경(River 맥의 Claude Code)은 egress가 열려 있어 에이전트가 직접 돌렸다.** 막힌 환경(Cowork 브릿지 셸 등)이면 River 터미널.
에이전트가 준비할 수 있는 것은 2026-09-11에 끝냈다(아래 「이미 해 둔 것」).

> **2026-09-17 적용 완료.** `BBOX = [-25, 12, 75, 62]`. `npm run fetch-external`(캐시 532 MB) → `npm run adapt` → `npm run validate` 218 통과. relief 6000×3000, 영토 버킷 23개 7,592 피처. 그 뒤에 **`npm run finish`(영역 마감, OVERHAUL §3.6c)** 를 돌린다. 이 파일의 나머지는 다음에 범위를 또 바꿀 때의 절차다.
...
> **용량 표는 [BACKLOG.md](BACKLOG.md) §A가 정본이다.** 여기 사본을 두면 River가 최종 숫자를 바꿀 때 둘을 손으로 맞춰야 한다.
> 실측 한 줄만 적는다: 지금 범위 `[-15,20,65,60]`의 DEM z0~7은 **840장 53MB**(2026-09-16 `public/datasets/rome/terrain/` 실측). 나머지 행은 추정이고 BACKLOG에 있다.
...
> 먼저 아래를 돌려야 한다([TASKS.md](TASKS.md) 0.1, 멱등이고 백업을 자동으로 뜬다).
>
> ```
