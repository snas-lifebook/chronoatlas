...
- 착수 전 실측 셋이 설계를 정했다. ① 초기 JS가 예산 400 kB gz 중 399.6이라 **무엇을 넣든 번들 분할이 먼저다.** ② 라이브에 DEM 타일이 없다(`terrain/meta.json` 404). 입체 보기는 로컬에서만 산다. ③ **이 작업 환경은 egress가 열려 있다.** 문서가 「River 터미널」로 적어 둔 재베이크·DEM 굽기를 에이전트가 직접 돌린다. River 손이 남는 것은 정본 재작성 승인뿐이고, 그것도 2026-09-17에 받았다(§3.11).
...
**완료 조건.** 로마·알레시아·루비콘 z11 캡처에서 육지 단색 픽셀 비율 10% 미만(`readPixels`로 잰다). z9 이탈리아 반도 캡처에 음영과 고도색이 보인다. `data/insets.json`에 한 줄 더하면 굽기부터 렌더까지 코드 수정 0.
...
**파이프라인** `scripts/finish-territory.py`(shapely, 빌드타임, adapt 뒤에 `layers/territory/*.geojson`을 다시 쓴다. 정본 JSONL은 안 건드린다).
1. **부드럽게**: Chaikin 2회 → `simplify(50 m)`. 톱니가 사라지고 정점 수는 비슷하다.
2. **해안 스냅**: 육지 마스크(NE 10m `land` + `minor_islands`)와 교집합. 그 전에 폴리곤을 300 m 버퍼해 곶·반도가 마스크 안에 들어오게 한다(R33 「곶이 안 칠해진다」).
3. **섬 귀속**: 어느 폴리티에도 안 덮인 NE 섬은 **가장 가까운 폴리티 해안에서 40 km 이내**면 그 폴리티에 붙인다. 규칙은 문서와 피처 속성(`island_rule: "nearest<=40km"`)에 남긴다. 사실 주장이 아니라 렌더 규칙이다. 큰 섬(시칠리아·크레타·키프로스)은 데이터가 이미 가진다.
4. **겹침 정리**: 인접 폴리티가 겹치면 면적 큰 쪽이 양보(BACKLOG C).

**렌더 마감**(`style`·엔진): 채움 55% + **안쪽 후광**(같은 색, `line-offset` 음수 + `line-blur`로 경계 안쪽 8 px 그라데이션) + 바깥선 1 px 세력색 + **해안선은 항상 맨 위에 잉크 1 px**. 색이 바다로 새 보이지 않는다. 미시 축척에서 territory 채움을 빼는 규칙은 그대로.
