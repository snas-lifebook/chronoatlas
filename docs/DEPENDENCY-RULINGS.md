...
> **2026-09-16 재분류.** 이 문서를 `docs/history/`(끝난 기록)에 넣었다가 되돌렸다. 기록이 아니라 **살아 있는 기각 근거**다.
>
> 실측한 현재 상태는 이렇다. 후보 9개 중 **권장 2건(1번 flyTo · 3번 원정로 행군)은 이미 구현됐다**(`src/map/engine.ts`·`src/token3d.ts`·`src/app/App.tsx`, 장면 19개 전부 `center` 보유). 2번 hover도 됐고, 4번은 발표 모드(`src/present.ts`, `?present=1`)로 절반 들어왔다. **`BACKLOG.md`를 거치지 않고 구현됐다** — 그래서 원장에는 이 후보들의 R 번호가 하나도 없다.
>
> 아직 살아 있는 것은 둘이다. **① 6번 family-chart 계보**는 미도입이다(7번째 데이터 축이라 두 번째 데이터셋 단계로 미뤘다). **② 7·8·9번의 보류·기각 근거**는 지금도 유효하다. `package.json`에 `deck.gl`·`d3`·`scrollama`·`family-chart`가 **전부 없고**, 이 문서가 그 이유를 들고 있는 유일한 자리다. 새로 들어온 에이전트가 "관계 그래프에 d3를 쓰자"거나 "궤적은 deck.gl TripsLayer로 하자"고 제안하기 전에 여기를 읽어야 한다.
>
> 아래 본문은 2026-08-18 원문 그대로다. 「비주얼파이프라인 현재 상태」 절의 데이터 규모(정착지 9·전투 6·원정로 11)는 그때 값이고 지금은 훨씬 크다. 코드 라인 번호도 낡았다. **판단 근거만 유효하고 수치는 낡았다고 읽어라.**


> 볼트 `Works/비주얼파이프라인/`에서 이관 (2026-09-16). 원본 작성 2026-08-18 · 최종 ?
