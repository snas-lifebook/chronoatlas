...
| 말 | 뜻 | 주의 |
|---|---|---|
| **정본** | 단일 진실 원천(canonical source). 볼트의 `ontology/*.jsonl`(데이터)과 `docs/`의 설계 문서 | "원본"이 아니다. **에이전트가 쓰지 못하는 것**이라는 함의가 붙어 있다 |
...
> **2026-09-16 정정.** 여기 있던 「지금 상태 스냅샷 (2026-09-12)」을 걷어냈다. `HANDOFF.md` §1과 같은 것을 두 곳에 적고 있었고, **둘 중 하나만 갱신되면서 이쪽이 하루 뒤처졌다.** 9/13에 들어온 카이사르 팩 작업(장면 19 · 교보재 19파일 · 검수 하네스 · R42~R45 · 미시 지도 셋 · 주변 민족 · 평야)이 통째로 빠져 있었다. 이 문서 스스로 *"숫자를 여기 적지 않는다, 한 커밋마다 낡는다"*고 적어 놓고 라운드 상태는 적고 있었던 것이다.

현재 상태는 **[HANDOFF.md](HANDOFF.md) §1**, 무엇이 남았는지는 **[BACKLOG.md](BACKLOG.md) 「지금 순서」**, 발표 팩 슬라이스는 **[PACK-CAESAR.md](PACK-CAESAR.md) §0**이 진다. 셋 다 이 문서보다 자주 갱신된다.

아래 둘은 상태가 아니라 **판단 근거**라서 여기 남긴다. 다른 문서가 이유를 안 들고 있다.

**팔레트 승인 대기 (`proposals/`)** — 세력 3색 페르시아 `#2F5DA8` · 이슬람세력 `#8E3A6B` · 유목민 `#6E7A22`(승인 전까지 셋 다 회색). 내전 2색 카이사르파 `#8C3B2E` · 폼페이우스파 `#5B6E8C`. **「반란세력」 회색을 안 쓴 이유는 어느 쪽이 반란인지가 이 전쟁의 쟁점이라서다.** 색이 논점을 선점하면 안 된다.

**말판 두 판의 사료** — 칸나이 BC216은 폴리비오스 3.113~117, 파르살루스 BC48은 카이사르 『내전기』 3.88~99. 말판 배치는 사실 주장이 아니라 교보재이므로 `teaching: true`를 강제하고 신뢰도 체계에 섞지 않는다(CONSTITUTION).
