...
## 5. 지금 상태 스냅샷 (2026-09-12)

로컬 HEAD와 원격 차이는 `git log --oneline -5` · `git rev-list --count origin/main..HEAD`로 직접 보라. **숫자를 여기 적지 않는다** — 한 커밋마다 낡는다.

**닫힌 라운드**: E(북마크) ● · F(그 해의 인물·국가·사건) ● · H(데이터 결손) ◐ 코드만 · G 2D 렌더 ● (R37) · 인물 위치 ● (R38). G 전체는 ◐
...
**결재 대기**: `proposals/` 7장 — 연도 3건 · 악티움 place · resource/terrain 7건 · Pleiades 연결 8건 · rel 오용 11건 · 팔레트 두 벌. 팔레트는 **세력 3색**(페르시아 `#2F5DA8` · 이슬람세력 `#8E3A6B` · 유목민 `#6E7A22` — 승인 전까지 셋 다 회색)과 **내전 2색**(카이사르파 `#8C3B2E` · 폼페이우스파 `#5B6E8C` — 파르살루스 말판이 이걸 기다린다. 「반란세력」 회색을 안 쓴 이유는 어느 쪽이 반란인지가 이 전쟁의 쟁점이라서다).

**말판 판 둘**: 칸나이 BC216(폴리비오스 3.113~117) · 파르살루스 BC48(카이사르 『내전기』 3.88~99). 파르살루스는 장면 등록(`data/scenes/rome.json`, `group: "말판"`)이 남았다.
