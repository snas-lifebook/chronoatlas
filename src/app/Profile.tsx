...
export function Profile({ base, routeId, path }: { base: string; routeId: string; path: [number, number][] }) {
  ...
  return (
    <div className="ins-profile">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} role="img" title={`AWS Terrain Tiles(SRTM·GMTED2010 외) · 96점 리샘플${seaPct > 0 ? ' · 해상 구간은 0m' : ''}`} aria-label={`경로 고도 단면. 총 ${Math.round(km)}킬로미터, 최고 ${Math.round(hi)}미터, 최저 ${Math.round(lo)}미터.`}>
