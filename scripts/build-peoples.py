...
# 사르마티아(유럽): 서=프루트(+카르파티아 북쪽은 직선 연장), 남=다누비우스 하류·흑해·아조프
# 해안선, 동=타나이스+케르치 해협, 북=52°N 박스 변.
s_box = box(23.0, 43.0, 41.0, 52.0)
...
# 아시아 사르마티아(아오르시·시라케스): 서=타나이스, 남=쿠반+테레크선, 동=볼가.
...
aorsi = clean(aorsi, land_in, [canon_u, bosporan, sarmatia])
...
FEATURES = [
    ...
    (sarmatia, {
        ...
        "source": "스트라본 7.3.17(록솔라니가 타나이스와 보리스테네스 사이 평원에, 이아지게스 사르마타이가 "
                  "그 서쪽에) + 11.1.5(타나이스가 유럽과 아시아를 가른다). 기하: Natural Earth rivers "
                  "프루트(서)·다누비우스 하류(남서)·타나이스=돈(동) 정점 복사, 남쪽은 land.geojson "
                  "흑해·아조프 해안선, 동남 이음선은 돈 하구~케르치 해협(킴메리아 보스포루스) 직선으로 "
                  "아조프 해 위. 프루트 수원 북쪽(48.3°N~52°N, 약 520km)은 직선 연장이고 "
                  "북쪽 52°N은 교보재용 절단선이다 — 고대 사료에 북쪽 한계가 없다. "
                  "프루트~드니스테르 사이(바스타르나이·티라게타이)도 이 면에 들어간다.",
    ...
    (aorsi, {
        ...
        "valid_from": -60, "valid_to": -27,
        "confidence": "low",
        "source": "**알란은 기원전 60년에 없다** — 알라니는 서기 1세기 사료(세네카·루카누스·요세푸스)에 "
                  "처음 나온다. 같은 공간을 기원전 1세기 사료가 부르는 이름이 아오르시·시라케스다"
                  "(스트라본 11.5.8, 절수는 재확인 권장). 기하: Natural Earth rivers 타나이스=돈(서)·"
                  "쿠반(남)·볼가(동) 정점 복사. 카우카소스 북측 기슭은 쿠반 수원~카스피 직선 이음선"
                  "(테레크 강 대응, NE에 테레크 없음). 볼가 동쪽의 상부 아오르시는 뺐다.",
        "actor_reason": "위와 같음 — 부족 연합이라 정본 어휘에 대응 항목 없음 → 중립색.",
    }),
    (bosporan, {
        "id": "people:bosporan",
        "name_ko": "보스포루스 왕국",
